...
from .serializers import AboutPageSerializer, GalleryImageSerializer, TeamMemberSerializer, ValueSerializer
...
class AboutAPIView(APIView):
    ...
    def get(self, request):
        ...
        response = {
            'hero': {'title': about_data.get('hero_title', ''), 'description': about_data.get('hero_description', '')},
            'about_section': {
                'title': about_data.get('about_title', ''),
                'paragraphs': [about_data.get('paragraph_1', ''), about_data.get('paragraph_2', '')],
                'action_button': {'text': about_data.get('button_text', ''), 'link': about_data.get('button_link', '')},
            },
            'values': {'title': 'Наши ценности', 'items': values},
            'team': {
                'title': 'Наша команда',
                'members': team,
                'action_button': {'label': 'Присоединиться', 'link': '/contacts'},
            },
            'gallery_carousel': {'title': 'Галерея', 'images': [{'id': f"img_{img['id']}", **img} for img in images]},
        }
